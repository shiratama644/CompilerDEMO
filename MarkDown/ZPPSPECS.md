...
### 9章 モジュールと名前空間 (Modules and Namespaces)

Z++では、C/C++のヘッダファイルとソースファイルの分離に似た仕組みを提供し、コードの分割と再利用を促進します。

ただし、リンカの概念がないため、`#include` と `using namespace` の振る舞いは独自のものとなっています。

#### 9.1. モジュール化の2ステップ
Z++のモジュール化は、以下の2つのステップで構成されます。

*   **ステップ1 ファイルのインクルード:** `#include` を使い、他のファイルの内容を物理的に取り込みます。
*   **ステップ2 シンボルのインポート:** `using namespace` を使い、取り込んだファイルからどのシンボル（関数、クラスなど）を現在のスコープで使用可能にするかを指定します。

#### 9.2. ファイルのインクルード (`#include`)

`#include` は、指定されたファイルの内容をその場に展開するプリプロセッサ命令です。

*   **書式:** `#include "ファイルパス"`
    *   **動作:** コンパイラは、指定されたZ++ソースファイルを読み込み、その内容を `#include` が書かれた場所に**テキストとして挿入**します。

*   **重要なルール:**
    1.  インクルードされるファイルは、**それ単体で完結したコードである必要はありません**。
    2.  通常、インクルードされるファイル（例: `math.zpp`）には、**再利用したい関数やクラスの定義のみを記述**します。
    3.  インクルードされるファイルに記述された関数、`struct`/`class`、`const`定数などは、この時点では**まだ利用可能になっていません**。

#### 9.3. シンボルのインポート

##### 9.3.1. なぜインポートが必要か
`#include` で取り込まれたコード内のシンボルは、**デフォルトではカプセル化された状態**にあります。

*   **目的:** `using namespace` を使わずにインクルードしただけでは、意図しないグローバルな名前空間の汚染を防ぎます。
*   **動作:** どのシンボルを現在のファイルスコープに持ち込むかを、開発者が明示的に選択する必要があります。

##### 9.3.2. インポートの指定 (`using namespace`)
`using namespace` は、`#include` したファイルから、**特定のシンボル**または**すべてのシンボル**を現在のスコープに導入します。

*   **書式:** `using namespace シンボル名1, シンボル名2, ...;`

*   **動作:**
    *   `using namespace` がない場合、`#include` したファイル内のどのシンボルも**利用できません**。
    *   `using namespace` の後にシンボル名を指定した場合、**指定されたシンボルのみ**が利用可能になります。
    *   シンボル名を指定しない `using namespace;` は、**すべてのシンボル**をインポートするショートカットですが、非推奨です。

*   **特徴:**
    1.  **選択的インポート:** 必要な機能だけを選択してインポートすることで、名前の衝突を避けやすくなります。
    2.  **依存関係の明確化:** コードの冒頭で、どの外部機能をどのファイルから利用しているかが明確になります。

#### 9.4. コンパイルプロセス
コンパイラは、以下の3段階でソースコードを処理します。

1.  **フェーズ1 プリプロセス（インクルード）**
    *   ソースファイル内の `#include` をすべて処理し、一つの巨大なソースコード（翻訳単位）を生成します。

2.  **フェーズ2 シンボルテーブル構築**
    *   翻訳単位を解析し、`using namespace` を基に、各スコープでどのシンボルが利用可能かを決定します。
    *   **`using namespace` がない場合:** シンボルは定義されているが、どのスコープからも見えない状態になります。
    *   **`using namespace` がある場合:** `#include` されたファイルから対応するシンボルを探し、現在のスコープで利用可能にします。

3.  **フェーズ3 コード生成**
    *   `using namespace` で利用可能になったシンボルのみを参照して、アセンブリコードを生成します。
    *   利用されなかった関数やクラスは、**最終的なバイナリには含まれず**、ROM領域を消費しません（デッドコード除去）。

#### 9.5. 使用例

**`my_math.zpp`**
```zpp
// 再利用可能な add と sub の2つの関数を定義
int add(int a, int b) { return a + b; }
int sub(int a, int b) { return a - b; }
```

**例1: `using namespace` なし（すべてのシンボルをインポートする非推奨な方法）**
```zpp
// main_full_import.zpp

#include "my_math.zpp"
// using namespace; // 省略されているが、デフォルトで全インポートと解釈される

int main() {
    int result1 = 0;
    int result2 = 0;

    // my_math.zpp の内容がすべて利用可能
    // add と sub の両方が呼び出せる
    result1 = add(10, 5); // OK
    result2 = sub(10, 5); // OK
...
**例2: `using namespace` による選択的インポート**
```zpp
// main_selective_import.zpp

#include "my_math.zpp"

// using namespace で 'add' 関数のみをインポート
using namespace add;
