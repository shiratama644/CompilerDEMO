...
> **📖 参照:** クラスの詳細については [3.5. クラス (class)](#35-クラス-class---カプセル化のための型) を参照してください。
...
> **📖 参照:** 各演算子の詳細は [4章 式と演算子](#4章-式と演算子-expressions-and-operators) を参照してください。
...
1.  関連する機能（[関数](#7章-関数-functions)、[クラス](#35-クラス-class---カプセル化のための型)、定数など）を、`math.zpp`や`io.zpp`のような、意味のある単位の**ファイル（モジュール）**に分割します。
