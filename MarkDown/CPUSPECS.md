...
*   **メモリ空間:**
    *   **ROM (命令用):** 2KB (1024ワード × 16ビット/ワード)、10ビットアドレス。
    *   **RAM (データ用):** 256バイト、8ビットアドレス。
    *   **I/O ポート:** 入力/出力 各16個（各8ビット）、4ビットアドレス。
*   **ハードウェアスタック (CPU内蔵):**
...
*   **動作:** `GPRstack <- rA(apA)`
*   **フラグ更新:** No
*   **説明:** 汎用スタックにレジスタ`rA(apA)`の値を退避します。サブルーチン内で使用するレジスタの保存に利用します。

##### **POP** (Pop)
*   **書式:** `POP rA(apA)`
*   **動作:** `rA(apA) <- pop(GPRstack)`
*   **フラグ更新:** No
*   **説明:** 汎用スタックから値を取り出し、レジスタ`rA(apA)`に格納します。
